...
result = {
    ...
    # We only prove presence, never print values.
    ...
    # Lengths are safe enough for a PoC with dummy secrets, but still no values.
    "scw_access_key_length": env_len("SCW_ACCESS_KEY"),
    "scw_secret_key_length": env_len("SCW_SECRET_KEY"),

    # OIDC request variables are exposed only when id-token permission is available.
